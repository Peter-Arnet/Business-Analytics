def caesar_cipher_encrypt(str_to_encrypt, n):
    ...
    keymap_alpha = {alphabet[i]:i for i in range(len(alphabet))}
    #keymap_alpha = dict(zip('abcdefghijklmnopqrstuvwxyz', range(26)))
    
    encrypted = ''
    for char in str_to_encrypt:
        num = keymap_alpha[char]
        encrypted_num = (num + n) % 26 
        encrypted += alphabet[encrypted_num]
...
def caesar_cipher_decrypt(str_to_decrypt, n):
    ...
    # DON'T CHANGE ANYTHING ABOVE
    # YOUR CODE BELOW THIS
    decrypted = caesar_cipher_encrypt(str_to_decrypt, -n)
    return decrypted
