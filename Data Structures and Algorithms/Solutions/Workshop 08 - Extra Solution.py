...
from ws08_solution import greedy, Item
...
def greedy_fractional(items, max_weight, key_function):
    ...
    for i in range(len(sorted_items)):
        ...
        if total_weight + cur_item.get_weight() <= max_weight:
            result.append([cur_item, 1])
            total_weight += cur_item.get_weight()
            total_value += cur_item.get_value()
        else:  # If cannot fit full item, fill knapsack to capacity
            fraction_added = (max_weight - total_weight) / cur_item.get_weight()
            result.append((cur_item, fraction_added))
            total_weight += cur_item.get_weight() * fraction_added
            total_value += cur_item.get_value() * fraction_added
...
def greedy_heuristic(items, max_weight, key_function, max_players):
    ...
    for i in range(len(sorted_items)):
        cur_item = sorted_items[i]
        if total_weight + cur_item.get_weight() <= max_weight and pl < max_players:
            pl_pos = cur_item.get_position()
            if pl_pos == 'Goalkeeper' and gl < max_goal:
                gl += 1
                result.append(cur_item)
                pl += 1
                total_weight += cur_item.get_weight()
                total_value += cur_item.get_value()
            elif pl_pos == 'Defender' and df < max_def:
                df += 1
                result.append(cur_item)
                pl += 1
                total_weight += cur_item.get_weight()
                total_value += cur_item.get_value()
            elif pl_pos == 'Midfielder' and md < max_mid:
                md += 1
                result.append(cur_item)
                pl += 1
                total_weight += cur_item.get_weight()
                total_value += cur_item.get_value()
            elif pl_pos == 'Forward' and fw < max_fw:
                fw += 1
                result.append(cur_item)
                pl += 1
                total_weight += cur_item.get_weight()
...
max_players = 11
budget = 900
rs, v = greedy_heuristic(pl_items, budget, density, max_players)
